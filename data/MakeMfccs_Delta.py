...
def create_mfccs(path, target_sr, max_duration, hop_length, n_mfcc):
    try:
        ...
        # Stack to shape (3, n_mfcc, time)
        mfcc_stack = np.stack([mfccs, delta_mfcc, delta2_mfcc], axis=0)

        # Normalize each channel (optional but recommended)
        mfcc_stack = (mfcc_stack - np.mean(mfcc_stack)) / \
            (np.std(mfcc_stack) + 1e-6)

        length = int(max_duration * (target_sr / hop_length))

        # Zero-pad or truncate along the time axis (axis=2)
        if mfcc_stack.shape[2] < length:
            pad_width = length - mfcc_stack.shape[2]
            mfcc_stack = np.pad(
                mfcc_stack, ((0, 0), (0, 0), (0, pad_width)), mode='constant')
        else:
            mfcc_stack = mfcc_stack[:, :, :length]

        filename = os.path.basename(path)
        filename = os.path.splitext(filename)[0]  # Remove extension
        mfcc_delta_path = os.path.join(
            FOLDER_NAME, f"{filename}_mfcc_delta.pt")
        mfcc_tensor = torch.tensor(mfcc_stack, dtype=torch.float32)
        torch.save(mfcc_tensor, mfcc_delta_path)

        return {'path': path, 'mfcc_delta_path': mfcc_delta_path, 'status': 'success'}
    except Exception as e:
        error_msg = f"Error processing {path}: {str(e)}\n{traceback.format_exc()}"
        print(error_msg)
        return {'path': path, 'mfcc_delta_path': None, 'status': 'error', 'error': str(e)}
...
def main():
    ...
    if SAVE_CSV:
        ...
        path_to_spec = {r['path']: r['mfcc_delta_path'] for r in results}

        # Update the dataframes
        train['Mfccdeltapath'] = train['Filepath'].map(path_to_spec)

        # Save the updated dataframes
        train = train[['Filepath', 'Mfccdeltapath', 'Emotion']]
        train.to_csv('mfcc_delta_train_dataset.csv', index=False)
    ...
    if SAVE_CSV:
        # Create a mapping from paths to spectrograms
        val_path_to_spec = {r['path']: r['mfcc_delta_path']
                            for r in val_results}

        # Update the test dataframe
        val['Mfccdeltapath'] = val['Filepath'].map(val_path_to_spec)

        # Save the updated test dataframe
        val = val[['Filepath', 'Mfccdeltapath', 'Emotion']]
        val.to_csv('mfcc_delta_val_dataset.csv', index=False)
    ...
    if SAVE_CSV:
        # Create a mapping from paths to spectrograms
        test_path_to_spec = {r['path']: r['mfcc_delta_path']
                             for r in test_results}

        # Update the test dataframe
        test['Mfccdeltapath'] = test['Filepath'].map(test_path_to_spec)

        # Save the updated test dataframe
        test = test[['Filepath', 'Mfccdeltapath', 'Emotion']]
        test.to_csv('mfcc_delta_test_dataset.csv', index=False)
    ...
    print(f"Results saved to mfcc_delta_test_dataset.csv")
