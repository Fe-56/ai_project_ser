...
def augment_speech(path):
    try:
        ...
        y = OfflineDataAugmentation.time_stretch(
            y, stretch_rate_range=STRETCH_RATE_RANGE)

        filename = os.path.basename(path)
        if filename.endswith('.wav'):
            filename = filename.replace('.wav', '_time.wav')
        elif filename.endswith('.mp4'):
            filename = filename.replace('.mp4', '_time.wav')

        time_path = os.path.join(FOLDER_NAME, filename)
        sf.write(time_path, y, sr)

        return {'path': path, 'time_path': time_path, 'status': 'success'}
    except Exception as e:
        error_msg = f"Error processing {path}: {str(e)}\n{traceback.format_exc()}"
        print(error_msg)
        return {'path': path, 'time_path': None, 'status': 'error', 'error': str(e)}
...
def main():
    ...
    if SAVE_CSV:
        ...
        path_to_time = {r['path']: r['time_path'] for r in results}

        # Update the dataframes
        train['Timepath'] = train['Filepath'].map(path_to_time)

        # Save the updated dataframes
        train = train[['Filepath', 'Timepath', 'Emotion']]
        train.to_csv('augmented_time_train_dataset.csv', index=False)
    ...
    print(f"Results saved to augmented_time_train_dataset.csv")
