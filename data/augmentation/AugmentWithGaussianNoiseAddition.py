...
SAVE_CSV = True
FOLDER_NAME = 'augmented_noise'
NOISE_FACTOR_RANGE = (0, 0.5)
...
def augment_speech(path):
    try:
        ...
        y = OfflineDataAugmentation.add_gaussian_noise(
            y, noise_factor_range=NOISE_FACTOR_RANGE)

        filename = os.path.basename(path)
        if filename.endswith('.wav'):
            filename = filename.replace('.wav', '_noise.wav')
        elif filename.endswith('.mp4'):
            filename = filename.replace('.mp4', '_noise.wav')

        noise_path = os.path.join(FOLDER_NAME, filename)
        sf.write(noise_path, y, sr)

        return {'path': path, 'noise_path': noise_path, 'status': 'success'}
    except Exception as e:
        error_msg = f"Error processing {path}: {str(e)}\n{traceback.format_exc()}"
        print(error_msg)
        return {'path': path, 'noise_path': None, 'status': 'error', 'error': str(e)}
...
def main():
    ...
    if SAVE_CSV:
        # Create a mapping from paths to noisy speech
        path_to_noise = {r['path']: r['noise_path'] for r in results}

        # Update the dataframes
        train['Noisepath'] = train['Filepath'].map(path_to_noise)

        # Save the updated dataframes
        train = train[['Filepath', 'Noisepath', 'Emotion']]
        train.to_csv('augmented_noise_train_dataset.csv', index=False)
    ...
    print(f"Results saved to augmented_noise_train_dataset.csv")
